...
function validatePersonalityConfig(config: any): PersonalityConfig | null {
  ...
  const isValid = fields.every(field => 
    typeof field === 'number' && field >= 1 && field <= 10
  );
...
const resources: Resource[] = PERSONALITY_PRESETS.map(preset => ({
  ...
  try {
    switch (name) {
      case 'inject_personality': {
        const { text, personality, apiKey } = args as {
          text: string;
          personality: any;
          apiKey?: string;
        };
      ...
      case 'inject_preset_personality': {
        const { text, presetId, apiKey } = args as {
          text: string;
          presetId: string;
          apiKey?: string;
        };
      ...
      case 'simulate_response': {
        const { prompt, personality, apiKey } = args as {
          prompt: string;
          personality: any;
          apiKey?: string;
        };
      ...
      case 'list_personality_presets': {
        ...
        if (category) {
          filteredPresets = getPresetsByCategory(category as PersonalityPreset['category']);
        }
        ...
        const presetList = filteredPresets.map(preset => 
          `**${preset.name}** (${preset.id})\n` +
          `Category: ${preset.category}\n` +
          `Description: ${preset.description}\n` +
          `Tags: ${preset.tags.join(', ')}\n` +
          `Personality: O:${preset.config.openness} C:${preset.config.conscientiousness} E:${preset.config.extraversion} A:${preset.config.agreeableness} N:${preset.config.neuroticism}`
        ).join('\n\n');
